  ...
  if @current_resource.exists
    converge_by("uninstall package #{ @current_resource.package }") do
      execute "uninstall package #{@current_resource.package}" do
        command "#{node[:chocolatey][:bin]} uninstall  #{@new_resource.package} #{cmd_args}"
      end
    end
  else
...
def package_installed?(name)
  !Mixlib::ShellOut.new("#{node[:chocolatey][:bin]} version #{name} -localonly #{cmd_args}").run_command.error?
end

def package_exists?(name, version)
  if package_installed?(name)
    if version
      cmd = Mixlib::ShellOut.new("#{node[:chocolatey][:bin]} version #{name} -localonly #{cmd_args}")
      cmd.run_command
      software = cmd.stdout.split("\r\n").each_with_object({}) do |h, s|
        v, k = s.split
...
def upgradeable?(name)
  if @current_resource.exists
    ...
    cmd = Mixlib::ShellOut.new("#{node[:chocolatey][:bin]} version #{name} #{cmd_args}")
    cmd.run_command
...
def install(name)
  execute "install package #{name}" do
    command "#{node[:chocolatey][:bin]} install #{name} #{cmd_args}"
  end
end

def upgrade(name)
  execute "updating #{name} to latest" do
    command "#{node[:chocolatey][:bin]} update #{name} #{cmd_args}"
  end
end

def install_version(name, version)
  execute "install package #{name} to version #{version}" do
    command "#{node[:chocolatey][:bin]} install #{name} -version #{version} #{cmd_args}"
  end
end
